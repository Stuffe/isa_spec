...
when isMainModule:
  ...
  let file_name = $args.spec_file
  let spec_source = readFile(file_name)
  let spec_result = parse_isa_spec(file_name, spec_source)
  if spec_result.error.message != "":
    quit "'" & file_name & "' error: " & spec_result.error.message & " at line " &
      $spec_result.error.line & ".", 1
  for asm_file in args.asm_files:
    ...
    if asm_result.top_file_errors.len > 0:
      quit "'" & $asm_file & "' error: " & asm_result.top_file_errors[0].message & " at line " &
        $asm_result.top_file_errors[0].line & ".", 1
