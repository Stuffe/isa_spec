...
func new_stream_slice*(source: string): StreamSlice =
  var reference = new(string)
  reference[] = source
  return StreamSlice(source: reference, start: 0, finish: source.len)
...
converter toTokenString*(t: Token): TokenString =
  TokenString(s: $t.s, tk: t.tk)
...
func restore*(s: var StreamSlice, cp: RestorePoint) =
  {.noSideEffect.}:
    ...
    assert (s.source == tracked_source) == (cp.token_count >= 0),
      "Token tracking status changed since this RestorePoint was created"
...
func restore_with_tokens*(s: var StreamSlice, cp: RestorePoint): seq[Token] =
  {.noSideEffect.}:
    assert s.source == cp.s.source, "RestorePoint does not apply to this stream"
    assert (s.source == tracked_source) == (cp.token_count >= 0),
      "Token tracking status changed since this RestorePoint was created"
    ...
    s = cp.s
    if s.source == tracked_source:
      if cp.token_count > 0 and tokens[cp.token_count - 1].s.finish != cp.last_token_finish:
        result.add(tokens[cp.token_count - 1])
      for i in cp.token_count ..< tokens.len:
        result.add(tokens[i])
...
func add_token*(s: StreamSlice, tk: TokenKind) =
  ...
  {.noSideEffect.}:
    ...
    if tk in CONTROL_TOKENS:
      assert start == finish
      tokens.add Token(
        s: StreamSlice(source: s.source, start: start, finish: finish), tk: tk
      )
      return
