...
func parse_unsigned*(s: StreamSlice): (string, uint64) =
  ...
  let len =
    if s.len < 3:
      ...
      case s[1]:
        of 'x': parseHex($s[2..^1], num)
        of 'o': parseOct($s[2..^1], num)
        of 'b': parseBin($s[2..^1], num)
        else: parseBiggestUInt($s, num)
  if len == 0:
...
func get_size*(s: var StreamSlice): (string, uint64, bool) =
  ...
  let s_number = get_unsigned(s).on_err do:
    result[0] = err
    return
  ...
  let number = parse_unsigned(s_number).on_err do:
    result[0] = err
    return
...
func parse_parametrized_pattern*(s: var StreamSlice, parameters: seq[StreamSlice]): (seq[string], seq[int]) =
  var slice = s
