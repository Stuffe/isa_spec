...
when existsEnv("CI"):
  # if we are running in Contious Integeration testing (e.g. Github actions), run all tests
  ...
  const RUN_SUBTESTS: seq[string] =
    @[]
      # E.g. @["x86_64", "x86_64.and"], both the spec name and the spec name + subtest id need to be listed.
  const SKIP_TESTS: seq[string] = @["aarch64", "aarch64_neon"]
  const GENERATE_TOKEN_LIST = false
    # If true, all tests that get run and have a [isa_]tokens file get the "golden" set of tokens output
  const MANY_SUBTESTS_THRESHOLD = 10

type AsmTestFile =
  tuple[
    source_file: string, error_file: string, result_file: string, lines_file: string
  ]
...
for (kind, test_dir) in TEST_PATH.walk_dir():
  ...
  if test_name.startswith('_'):
    continue
  if (RUN_SINGLE_TEST != "" and test_name != RUN_SINGLE_TEST) or
      (test_name != RUN_SINGLE_TEST and test_name in SKIP_TESTS):
    continue
  ...
  for (kind, file_name) in test_dir.walk_dir(relative = true):
    ...
    if parts.len notin [2, 3]:
      raiseAssert(
        &"Tests file names should have format `{{name}}[.{{subid}}].{{kind}}` ({test_dir/file_name})"
      )
    let name = parts[0]
    ...
    of "isa_err":
      doAssert parts.len == 2,
        &"isa error files can't have a subid ({test_dir/file_name})"
  ...
  for sub_name, tests in sub_tests:
    if tests.spec_file == "":
      # If we don't have a spec file, assume that these are include related files
      continue
      ...
      for i, token in spec_result.tokens:
        ...
        when GENERATE_TOKEN_LIST:
          ...
          if i > expected_tokens.high:
            fail(
              test_name, tests.spec_file, &"To many tokens: Got extra\n{expected_line}"
            )
            break
          if expected_tokens[i] != expected_line:
            ...
            if error_count > 10:
              fail(
                test_name,
                tests.spec_file,
                &"Too many incorrect tokens, not checking further",
              )
              break
      when GENERATE_TOKEN_LIST:
        write_target.close()
    ...
    if spec_result.error.message != "":
      if tests.spec_error_file != "":
        let expected_error = readFile(tests.spec_error_file)
        var expected_line: int
        var expect_message: string
        block BLK_PARSE_EXPECTED_ERROR:
          for idx_c in 0 .. expected_error.high:
            if expected_error[idx_c] == ',':
              expect_message = expected_error[idx_c + 1 .. ^1]
              break
            if expected_error[idx_c] notin {'0' .. '9'}:
              continue
            expected_line = expected_line * 10 + ord(expected_error[idx_c]) - ord('0')

        var fail_msg: string
        if expected_line != spec_result.error.line:
          fail_msg &=
            &"Wrong line number: Got {spec_result.error.line}, Expected {expected_line}. "
        if expect_message != spec_result.error.message:
          fail_msg &=
            &"Wrong error message: Got \"{spec_result.error}\", Expected \"{expected_error}\". "
        if fail_msg != "":
        ...
        fail(
          test_name,
          tests.spec_file,
          &"Unexpected Spec failure: \"{spec_result.error}\"",
        )
      continue
    if tests.spec_error_file != "":
    ...
    for sid, asm_test in tests.asm_tests:
      ...
      if RUN_SUBTESTS.len > 0 and &"{test_name}.{sid}" not_in RUN_SUBTESTS:
        continue
      sub_fail = false
      doAssert asm_test.source_file != "",
        &"no '.asm' file for {test_name}/{sub_name}.{sid}"
      let asm_source = readFile(asm_test.source_file)

      var instructions = -instruction_count
      var time_take = -timer()
      let asm_result = assemble(
        TEST_PATH, asm_test.source_file.relative_path(TEST_PATH), isa_spec, asm_source
      )
      time_take += timer()
      ...
      if asm_result.errors.len != 0:
        if asm_test.error_file != "":
          let expected_errors = readFile(asm_test.error_file).split_lines()
          for i, expected_error in expected_errors:
            if i > asm_result.errors.high:
              fail(
                test_name,
                asm_test.source_file,
                &"Got {asm_result.errors.len}, Expected {expected_errors.len} error messages.",
              )
            ...
            block BLK_PARSE_EXPECTED_ERROR:
              ...
              for idx_c in 0 .. expected_error.high:
                if expected_error[idx_c] == ':':
                  expected_file = expected_error[0 ..< idx_c]
                  jdx_c = idx_c + 1
                  break

              for idx_c in jdx_c .. expected_error.high:
                if expected_error[idx_c] == ',':
                  expect_message = expected_error[idx_c + 1 .. ^1]
                  break
                if expected_error[idx_c] notin {'0' .. '9'}:
                  continue
                expected_line =
                  expected_line * 10 + ord(expected_error[idx_c]) - ord('0')

            var fail_msg: string
            if expected_file != asm_result.errors[i].loc.file:
              fail_msg &=
                &"Wrong file name: Got {asm_result.errors[i].loc.file}, Expected {expected_file}. "
            if expected_line != asm_result.errors[i].loc.line:
              fail_msg &=
                &"Wrong line number: Got {asm_result.errors[i].loc.line}, Expected {expected_line}. "
            if expect_message != asm_result.errors[i].message:
              fail_msg &=
                &"Wrong error message: Got \"{asm_result.errors[i].message}\", Expected \"{expect_message}\". "
            if fail_msg != "":
              fail(test_name, asm_test.source_file, fail_msg)
              break
          doAssert asm_test.result_file == "",
            &"Can't have both error and results for the same test ({asm_test.source_file}))"
        else:
          fail(
            test_name,
            asm_test.source_file,
            &"Unexpected assemble failure: {asm_result.errors}",
          )
        continue
      if asm_test.error_file != "":
        ...
        continue
      doAssert asm_test.result_file != "",
        &"Missing result file for {asm_test.source_file}"
      ...
      if asm_test.lines_file != "":
        ...
        for line_text in splitLines(line_source):
          doAssert line_text.split(':').len == 2,
            &"Each line in a `.lines` file should have the form 'file.asm:n' ({asm_test.lines_file})"
          let expected_fl = FileLocation(
            file: line_text.split(":")[0], line: parseInt(line_text.split(":")[1])
          )
          ...
          if expected_fl != actual_fl:
            fail(
              test_name,
              asm_test.source_file,
              &"Mismatch in expected line number for byte {byte_index}. " &
                &"Expected: {expected_fl}, got {actual_fl}",
            )
            break
          if last_line != expected_fl:
            let actual_start_index =
              asm_result.line_info.get_byte_from_line(expected_fl)
            if byte_index not_in actual_start_index and expected_fl.line != 0:
              fail(
                test_name,
                asm_test.source_file,
                &"Incorrect result from `get_byte_from_line({expected_fl})`: " &
